...
def _local_app_absolute_uri(request, path: str) -> str:
    host = (request.get_host() or "").split(":")[0]
    if host in {"localhost", "127.0.0.1"}:
        return f"https://localhost:4433{path}"
...
def _assistant_scope(user):
    ...
    approver_assistant = None
    if not supervisor:
        try:
            approver_assistant = ApproverAssistant.objects.select_related("supervisor", "supervisor__approver_profile").filter(
                assistant=user
            ).first()
            if approver_assistant and approver_assistant.supervisor:
                supervisor = approver_assistant.supervisor
        except Exception:
            approver_assistant = None
...
def _serialize_assistant_individual_request(req: ClearanceRequest):
    ...
    if not college and faculty_user:
        user_roles = getattr(faculty_user, 'roles_payload', [])
        if user_roles and len(user_roles) > 0:
            college = user_roles[0].get('college', '') or ''
            department = user_roles[0].get('department', '') or ''
...
def _login(request, user):
    ...
    print(f"SESSION: logging in -> session_key={request.session.session_key} user_id={user.id} email={user.email} role_value={role_value}")
...
def _get_authenticated_user(request):
    ...
    # Fallback: allow JWT auth for endpoints that rely on _get_authenticated_user.
    # Supports Authorization: Bearer <token> or a ?token=<token> query param.
    try:
...
def _validate_and_redirect_by_role(intended_role: str, user_roles: list) -> str | None:
    ...
    dashboard_urls = {
        ...
        'ciso': '/system-admin-dashboard',
        'ovphe': '/analytics-admin-dashboard'
    }
...
def google_oauth_callback(request):
    ...
    if not intended_role:
        ...
        login_url = _local_app_absolute_uri(request, f"/login?token={jwt_token}")
...
def _can_approver_access_request(user: User, clearance_request: ClearanceRequest) -> bool:
    ...
    approver_step = getattr(requirement, "approver_flow_step", None)
    approver_step_office = getattr(approver_step, "office", None) if approver_step else None

    if approver_profile.office and approver_step_office and approver_step_office == approver_profile.office:
        return True

    if approver_profile.college:
        if (
            faculty
            and (approver_profile.college == faculty.college)
            or requirement.target_colleges.filter(id=approver_profile.college.id).exists()
        ):
            return True
    ...
    if approver_profile.department:
        if (
            faculty
            and (approver_profile.department == faculty.department)
            or requirement.target_departments.filter(id=approver_profile.department.id).exists()
        ):
            return True
...
def _approver_archived_requests_for_archived_clearance(user: User, archived: ArchivedClearance):
    """Serialize archived clearance requests for the approver archived-individual API."""
    if not archived:
        return []
    ...
    if not timeline or not faculty:
        return []
...
def _legacy_faculty_dashboard_api_v1(request):
    ...
    if academic_year and term:
        clearance = Clearance.objects.filter(faculty=faculty, academic_year=academic_year, term=term).order_by("-id").first()

    if timeline:
        timeline_requests = ClearanceRequest.objects.filter(faculty=faculty, clearance_timeline=timeline)
...
def ovphe_system_analytics_api(request):
    ...
    if timeline and timeline.archive_date:
        _ensure_archived_timeline_records(timeline)
        archived_clearances = ArchivedClearance.objects.filter(clearance_timeline=timeline, faculty_id__in=[item["id"] for item in faculty_items])
        ...
        faculty_rows = list(Faculty.objects.select_related("college", "department", "office").filter(id__in=[item["id"] for item in faculty_items]))
...
@csrf_exempt
def ciso_users_by_role_api(request):
    if request.method != "GET":
        return JsonResponse({"detail": "Method not allowed"}, status=405)
    role_name = (request.GET.get("role") or "").strip()
    if not role_name:
        return JsonResponse({"detail": "role query required"}, status=400)
    try:
        from .models import User, Role
        user_ids = list(
            User.objects.filter(
                userrole_set__role__name=role_name,
                userrole_set__is_active=True
            )
            .order_by("id")
            .values_list("id", flat=True)
        )
        return JsonResponse({"user_ids": user_ids, "role": role_name})
    except Exception as e:
...
def ciso_notifications_api(request):
    ...
    if request.method == "POST":
        ...
        is_read = bool(payload.get("is_read"))
        user_roles = payload.get("user_roles")
        user_ids = payload.get("user_ids")  # Accept direct user IDs (for per-user notifications)
        created_by_id_raw = payload.get("created_by_id")
        approver_id_raw = payload.get("approver_id")
        ...
        try:
            print(
                "[DEBUG] ciso_notifications_api POST received: "
                f"title={title!r} "
                f"user_roles_type={type(user_roles).__name__} user_roles_len={(len(user_roles) if isinstance(user_roles, list) else 'NA')} "
                f"user_ids_type={type(user_ids).__name__} user_ids_len={(len(user_ids) if isinstance(user_ids, list) else 'NA')} "
                f"user_roles={user_roles!r} user_ids={user_ids!r}"
            )
        except Exception:
            pass
        ...
        try:
            # Get target users - either from user_ids directly or expand user_roles
            target_users = []
            roles_by_user_id = {}
            
            if user_ids:
                try:
                    print(f"[DEBUG] ciso_notifications_api using user_ids path: user_ids={user_ids!r}")
                except Exception:
                    pass
                ...
                try:
                    print(f"[DEBUG] ciso_notifications_api using user_roles path: user_roles={user_roles!r}")
                except Exception:
                    pass
                # Expand user_roles to user IDs (original logic)
                from .models import Role
                for role_name in user_roles:
                    ...
                    if role:
                        ...
                        for u in users_with_role:
                            roles_by_user_id.setdefault(u.id, set()).add(str(role_name))
                        target_users.extend(users_with_role)
            ...
            try:
                print(
                    "[DEBUG] ciso_notifications_api expanded unique_users: "
                    f"count={len(unique_users)} ids={[u.id for u in unique_users]!r}"
                )
            except Exception:
                pass
            ...
            for target_user in unique_users:
                ...
                if user_ids:
                    # caller provided explicit user_ids; there may be no role context
                    role_label = "GENERAL"
                else:
                    # infer from expanded roles
                    inferred = roles_by_user_id.get(target_user.id) or set()
                    
                    if "CISO" in inferred:
                        role_label = "CISO"
                    elif "OVPHE" in inferred:
                        role_label = "OVPHE"
                    elif inferred:
                        role_label = sorted(inferred)[0]
            ...
            print(f"[DEBUG] Failed to create notifications: {e}")
            return JsonResponse({"detail": "Failed to create notifications"}, status=500)
...
def ciso_system_users_api(request):
    ...
    assistants = StudentAssistant.objects.select_related("user", "college", "department").order_by("id")
...
def _legacy_faculty_dashboard_api_v2(request):
    ...
    if clearance:
        ...
        if config:
            ...
            req_status_by_id = {cr.requirement_id: cr.status for cr in timeline_requests.select_related("requirement")}
...
def _can_approver_access_requirement(approver_profile, requirement):
    ...
    if is_college_dean:
        ...
        if requirement.recipient_scope in ['all', 'college']:
            ...
            return requirement.target_faculty.filter(department=department).exists()
...
@csrf_exempt
def assistant_approver_action_api(request):
    print(f"DEBUG: assistant_approver_action_api called with method: {request.method}")
    return JsonResponse({"detail": "Test - function exists"}, status=200)
...
def approver_activity_logs_api(request):
    ...
    if request.method == "POST":
        ...
        print(f"[DEBUG] Activity log POST - Received event_type: '{event_type}'")
        print(f"[DEBUG] Activity log POST - Available choices: {[c[0] for c in ActivityLog.EventType.choices]}")
        print(f"[DEBUG] Activity log POST - Received data: {data}")
        ...
        print(f"[DEBUG] Activity log POST - Details: {details}")
        print(f"[DEBUG] Activity log POST - Department: {data.get('department')}")
        print(f"[DEBUG] Activity log POST - College: {data.get('college')}")

        if not event_type:
            return JsonResponse({"detail": "event_type is required"}, status=400)

        if event_type not in {c[0] for c in ActivityLog.EventType.choices}:
            print(f"[DEBUG] Activity log POST - Event type '{event_type}' not found in choices!")
            return JsonResponse({"detail": "Invalid event_type"}, status=400)
        ...
        if event_type in ["approved_clearance", "rejected_clearance", "assistant_approved_clearance", "assistant_rejected_clearance", "individual_approved_clearance", "individual_rejected_clearance", "overridden_approved_clearance", "overridden_rejected_clearance"] and (request_id or details):
            try:
                ...
                if request_id:
                    ...
                    faculty_member_name = next((d.replace("Faculty Member: ", "").strip() for d in details if "Faculty Member:" in d), "")
                    print(f"[DEBUG] Activity log - Looking up faculty by name: '{faculty_member_name}'")

                    if faculty_member_name:
                        # Split name and search by first and last name
                        name_parts = faculty_member_name.split()
                        print(f"[DEBUG] Activity log - Name parts: {name_parts}")
                        if len(name_parts) >= 2:
                            first_name = name_parts[0]
                            last_name = " ".join(name_parts[1:])
                            print(f"[DEBUG] Activity log - Searching for first_name='{first_name}', last_name='{last_name}'")
                            faculty = Faculty.objects.filter(
                                first_name__iexact=first_name,
                                last_name__iexact=last_name
                            ).select_related('college', 'department').first()
                            print(f"[DEBUG] Activity log - Query result: {faculty}")
                        else:
                            print(f"[DEBUG] Activity log - Name has less than 2 parts, using full name search")
                            faculty = Faculty.objects.filter(
                                models.Q(first_name__iexact=faculty_member_name) | 
                                models.Q(last_name__iexact=faculty_member_name)
                            ).select_related('college', 'department').first()
                    else:
                        faculty = None
        ...
        # Handle supervisor - check if user exists in FC_approverassistant table for all approval event types
        supervisor = None
        if event_type in ["approved_clearance", "rejected_clearance", "individual_approved_clearance", "individual_rejected_clearance", "assistant_approved_clearance", "assistant_rejected_clearance", "assistant_individual_approved_clearance", "assistant_individual_rejected_clearance"]:
            try:
                from .models import ApproverAssistant
                # Check if user exists in FC_approverassistant table (use assistant field)
                approver_assistant = ApproverAssistant.objects.filter(assistant=user).first()

                if approver_assistant and approver_assistant.supervisor:
                    supervisor = approver_assistant.supervisor
                    print(f"[DEBUG] Activity log - Found supervisor from ApproverAssistant: {supervisor}")

                else:
                    print(f"[DEBUG] Activity log - No matching assistant ID found for user {user.id}, supervisor_id will be NULL")

            except Exception as e:
                print(f"[DEBUG] Activity log - Error finding supervisor: {e}")

        if event_type in ["approved_clearance", "rejected_clearance"] and (office is None or college is None or department is None):
            try:
                approver_profile = user.approver_profile
                if office is None:
                    office = getattr(approver_profile, "office", None)

                if college is None:
                    college = getattr(approver_profile, "college", None)

                if department is None:
                    department = getattr(approver_profile, "department", None)
        ...
        return JsonResponse({
                "detail": "Activity log created successfully",
                "id": obj.id,
            },
            status=201,
        )
...
def approver_archived_individual_api(request):
    ...
    if request.method == "POST":
        ...
        with transaction.atomic():
            ...
            # Activity log is now created by the frontend with complete data
            _archive_clearance_timeline_records(timeline)
    ...
    try:
        ...
        import traceback
        print("[ERROR] approver_archived_individual_api GET failed:", str(e))
        print(traceback.format_exc())
        return JsonResponse(
            {"detail": f"Internal server error: {str(e)}", "traceback": traceback.format_exc()},
            status=500,
...
def approver_individual_approval_api(request):
    ...
    if request.method == "POST":
        ...
        with transaction.atomic():
            clearance_request.status = (
                ClearanceRequest.Status.APPROVED if action == "approve" else ClearanceRequest.Status.REJECTED
            )
            clearance_request.approved_by = user
            clearance_request.approved_date = timezone.now()
            clearance_request.remarks = remarks
...
def assistant_approver_clearance_api(request):
    print(f"DEBUG: assistant_approver_clearance_api called with method: {request.method}")
    user = _get_authenticated_user(request)
    if not user:
        return JsonResponse({"detail": "Authentication required"}, status=401)

    # Check if user has Student Assistant role (regardless of primary role)
    user_roles = user.get_active_roles().values_list('role__name', flat=True)
    if 'Student Assistant' not in user_roles:
        return JsonResponse({"detail": "Student Assistant role required"}, status=403)
    ...
    if request.method == "GET":
        ...
        print(f"DEBUG: Assistant approver POST request from user: {user}")
        try:
            data = json.loads(request.body.decode("utf-8") or "{}")
        except Exception:
            return JsonResponse({"detail": "Invalid JSON"}, status=400)
        ...
        print(f"DEBUG: Request data: request_ids={request_ids}, action={action}")
        ...
        print(f"DEBUG: Active timeline: {active_timeline}")

        # Get all clearance requests that this assistant approver can access
        accessible_requests = _assistant_clearance_queryset(user, active_timeline)
        accessible_request_ids = [req.id for req in accessible_requests]

        print(f"DEBUG: Assistant can access {len(accessible_requests)} requests")
        print(f"DEBUG: Accessible request IDs: {accessible_request_ids}")
        print(f"DEBUG: Requested IDs: {request_ids}")

        # Filter the requested IDs to only those the assistant can access
        clearance_requests = ClearanceRequest.objects.filter(
            id__in=request_ids,
            clearance_timeline=active_timeline
        ).filter(id__in=accessible_request_ids)

        print(f"DEBUG: Found {len(clearance_requests)} matching requests")

        if len(clearance_requests) != len(request_ids):
            print(f"DEBUG: Permission denied - requested {len(request_ids)} but can only access {len(clearance_requests)}")
            return JsonResponse({"detail": "Some clearance requests not found or not accessible", "status": 404})
        ...
        with transaction.atomic():
            updated_requests = []
            # Handle supervisor - check if user exists in FC_approverassistant table
            supervisor = None
            try:
                approver_assistant = ApproverAssistant.objects.filter(assistant=user).first()
                if approver_assistant and approver_assistant.supervisor:
            ...
            for clearance_request in clearance_requests:
                ...
                ActivityLog.objects.create(
                    event_type=(
                        ActivityLog.EventType.ASSISTANT_APPROVED_CLEARANCE if action == "approve"
                        else ActivityLog.EventType.ASSISTANT_REJECTED_CLEARANCE
                    ),
                    user=user,
                    user_role="Assistant",
                    faculty=clearance_request.faculty,
...
def assistant_approver_individual_approval_api(request):
    ...
    # Get session user info for activity log
    ...
    ActivityLog.objects.create(
        ...
        event_type=(
            ActivityLog.EventType.ASSISTANT_INDIVIDUAL_APPROVED_CLEARANCE if action == "approve"
            else ActivityLog.EventType.ASSISTANT_INDIVIDUAL_REJECTED_CLEARANCE
        ),
...
def assistant_approver_archived_individual_api(request):
    ...
    supervisor = (_assistant_scope(user) or {}).get("supervisor")
    if not supervisor:
        return JsonResponse({"detail": "Permission denied"}, status=403)
    ...
    if request.method == "POST":
        ...
        if not _can_approver_access_request(supervisor, clearance_request):
            return JsonResponse({"detail": "Permission denied"}, status=403)
        ...
        with transaction.atomic():
            ...
            clearance_request.approved_by = user
            clearance_request.approved_date = timezone.now()
            clearance_request.save(update_fields=["status", "remarks", "approved_by", "approved_date"])
        ...
        return JsonResponse(
            {
                ...
                "message": f"Clearance request {action}d successfully",
                "status": _to_request_status(clearance_request.status),
                "archivedId": str(archived.id),
            }
        )

    faculty = archived.faculty
    faculty_user = getattr(faculty, "user", None)
    requests = _approver_archived_requests_for_archived_clearance(supervisor, archived)
