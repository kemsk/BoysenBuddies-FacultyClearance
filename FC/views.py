...
def _serialize_assistant_individual_request(req: ClearanceRequest):
    ...
    # Get college and department - try faculty first, then fallback to user profile or other sources
    college = ""
    department = ""
    ...
    # If faculty doesn't have college/department, try to get from user's roles or other sources
    if not college and faculty_user:
        # Try to get college from user's roles or profile
    ...
    # Final fallback - use meaningful values instead of empty strings
    if not college:
