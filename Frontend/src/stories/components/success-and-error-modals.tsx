...
export type DataDumpSkippedRow = {
  rowLabel: string;
  reason: React.ReactNode;
};

export type DataDumpSuccessModalProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  created: number;
  updated: number;
  skipped: number;
  skippedRows?: DataDumpSkippedRow[];
  title?: React.ReactNode;
  message?: React.ReactNode;
  continueLabel?: string;
  onContinue?: () => void;
...
export function DataDumpSuccessModal({
  open,
  onOpenChange,
  created,
  updated,
  ...
  onContinue,
  disableContinue,
}: DataDumpSuccessModalProps) {
  const finalTitle = title ?? "Success";
  const finalMessage =
    message ??
    (skipped > 0 ? "Finished with partial success – review skipped rows" : "Finished successfully");
  ...
  const rows = (skippedRows || []).slice(0);
  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          className={cn(
            "fixed left-1/2 top-1/2 z-50 w-[520px] max-w-[calc(100vw-3rem)] -translate-x-1/2 -translate-y-1/2",
            "rounded-xl bg-white shadow-xl",
            "focus:outline-none",
          )}
        >
          <div className="p-6">
            <div className="flex flex-col items-center text-center">
              <div className={cn("mt-2 flex h-24 w-24 items-center justify-center rounded-full", "bg-success")}>
                <Check className="h-12 w-12 text-white" strokeWidth={3} />
              </div>

              <div className="mt-6 text-3xl font-bold text-black">{finalTitle}</div>
              <div className="mt-3 text-lg text-gray-700">{finalMessage}</div>
            </div>

            <div className="mt-6 grid grid-cols-3 gap-4">
              <div className="rounded-lg bg-gray-50 px-4 py-4 text-center">
                <div className="text-4xl font-bold text-black">{updated}</div>
                <div className="text-sm font-semibold text-muted-foreground">Updated</div>
              </div>
              <div className="rounded-lg bg-gray-50 px-4 py-4 text-center">
                <div className="text-4xl font-bold text-black">{created}</div>
                <div className="text-sm font-semibold text-muted-foreground">Created</div>
              </div>
              <div className="rounded-lg bg-amber-50 px-4 py-4 text-center">
                <div className="text-4xl font-bold text-amber-700">{skipped}</div>
                <div className="text-sm font-semibold text-amber-700">Skipped</div>
              </div>
            </div>
                ...
                <div className="max-h-60 overflow-y-auto divide-y divide-gray-300 bg-white">
                  {rows.map((r, idx) => (
                    <div key={`${r.rowLabel}-${idx}`} className="flex gap-4 px-4 py-4">
                      ...
                      <div className="min-w-0 flex-1 text-sm text-gray-600">{r.reason}</div>
                    </div>
                  ))}
                </div>
              </div>
