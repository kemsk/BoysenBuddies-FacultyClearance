...
export default function OVPHENotification() {
  ...
  const markAllAsRead = React.useCallback(async () => {
    try {
      const r = await fetch("/admin/xu-faculty-clearance/api/ovphe/notifications", {
        method: "PUT",
        credentials: "include",
        headers: {
      ...
      if (!r.ok) {
        console.error("OVPHE notifications mark-as-read failed", r.status, text);
        return;
      }
  ...
  const markOneAsRead = React.useCallback(async (item: NotificationItemWithRole) => {
    ...
    try {
      ...
      const text = await r.text();
      if (!r.ok) {
        console.error("OVPHE notifications mark-one-as-read failed", r.status, text);
        return;
      }

      setItems((prev) => prev.map((it) => ((it as any).id === id ? { ...it, is_read: true } : it)));
    } catch (e) {
      console.error("OVPHE notifications mark-one-as-read threw", e);
    }
  }, []);
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        ...
        const text = await r.text();
        if (!r.ok) {
          console.error("OVPHE notifications fetch failed", r.status, text);
          setItems([]);
          return;
        }
        ...
        try {
          ...
          const safeItems = rawItems.map((it) => ({
            ...it,
            details: Array.isArray((it as any).details) ? (it as any).details : [],
          }));
          setItems(safeItems);
        } catch (e) {
          console.error("OVPHE notifications response was not JSON", r.status, text);
          setItems([]);
        }
      } catch (e) {
        console.error("OVPHE notifications fetch threw", e);
        setItems([]);
      }
    };
