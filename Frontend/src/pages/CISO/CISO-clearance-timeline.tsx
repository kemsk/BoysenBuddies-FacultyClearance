        ...
        if (options?.notifyUpdated) {
          ...
          if (madeInactive) {
            ...
            void postCISONotification({
              ...
              status: null,
              is_read: 0,
              user_roles: ["Approver", "Faculty", "Assistant"],
              created_by_id: null,
              approver_id: null,
              clearance_period_start_date: clearancePeriodStartDate,
              clearance_period_end_date: clearancePeriodEndDate,              
    ...
    (selected: StoredClearanceTimelineItem) => {
      void deleteTimeline({ id: selected.id, action: "archive" })
        .then(() => {
          ...
          openCreateResult("success", "Timeline Filed", `${selected.term} was filed successfully.`);
        })
        .catch((error) => {
          const message = error instanceof Error ? error.message : "Timeline filed failed.";
          openCreateResult("error", "Archive Filed", message);
        });
    },
